...
- @.Codex/memory/user_profile.md
- @.Codex/memory/project_context.md
- @.Codex/memory/project_structure.md
- @.Codex/memory/exam_guide_ref.md
- @STATE.md

Viajan con el repo (a diferencia de la auto-memoria de `~/.Codex/projects/...` que es local por Mac). Si se actualizan hechos, actualizar el archivo correspondiente.
...
3. **Skills** relevantes: `init`, `review`, `security-review`, `loop` (vienen con Codex).
...
Si falta algún MCP esperado, decirlo con el comando exacto para arreglarlo (ej. `Codex mcp add playwright -- npx -y @playwright/mcp@latest`) y preguntar al usuario si quiere que lo instale ahora o después.

No hacer este chequeo en mensajes posteriores — solo el primero de la sesión.

## Qué es este repositorio

**No es un proyecto de software.** Es la carpeta de apuntes y preparación para el examen **Codex Certified Architect – Foundations** de Anthropic. El contenido principal son notas en markdown (en español) de los cursos de Anthropic Academy más notebooks prácticos del curso *Building with the Codex API*.
...
- `01_Claude_Code_in_Action.md` — notas del curso Codex in Action (completado).
- `02_Claude_101.md` — notas del curso Codex 101 (completado).
- `Building_with_the_Claude_API.md` — notas del curso Building with the Codex API (**en curso**). Al inicio del archivo hay una línea `Progreso:` con el conteo de lecciones completadas; actualízala al terminar una lección.
- `Building with the Codex API/` — notebooks Jupyter prácticos del curso de API + `dataset.json` + `.env`.
...
La API key vive en `androipic certifiacion/Building with the Codex API/.env`:
...
Modelo usado en los ejercicios: `Codex-sonnet-4-0` (es lo que trae el curso; los modelos recientes son `Codex-sonnet-4-6` y `Codex-opus-4-7`, pero mantener lo que use la lección a menos que el usuario pida actualizar).

## Skills del proyecto

En `.Codex/skills/` viven skills custom que Codex carga automáticamente al abrir este repo:
...
Registrados en `~/.Codex.json` bajo la entrada de este proyecto:
...
Codex mcp add playwright -- npx -y @playwright/mcp@latest
# tras instalarlo hay que reiniciar Codex para que cargue el MCP

# 4. API key — copiar el template y pegar la key real
cp "androipic certifiacion/Building with the Codex API/.env.example" \
   "androipic certifiacion/Building with the Codex API/.env"
...
- El objetivo transversal es prep de examen. Al explicar conceptos, conectar con los dominios del examen (Codex básico, Codex & workflows, Codex API). Priorizar claridad didáctica sobre ingeniería.
